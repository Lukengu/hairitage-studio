@echo off
REM Windows port of seed-blogs.sh
...
set "JOB_NAME=hairitage-seed-blogs"

if not defined GCS_PUBLIC_BUCKET set "GCS_PUBLIC_BUCKET=0"
set "ENV_FILE=%TEMP%\seed-blogs-env-%RANDOM%%RANDOM%.yaml"
...
echo ==^> Deploying blog seed job %JOB_NAME%
...
echo ==^> Creating blog posts with images (skips duplicates)
...
echo.
echo Blog seed complete. Review posts at https://%DOMAIN%/blog.html
